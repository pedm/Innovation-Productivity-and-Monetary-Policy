...
% 32. Taylor Rule
exp(R_nom)  = ( exp(R_nom(-1)) )^gamma_r * ( (exp(pi)/pi_ss)^gamma_pi * ( exp(YD)/exp(YD_pot) )^gamma_y *  pi_ss/(beta*g_ss^(-1))  )^(1-gamma_r) * exp(MONSHK);
...
exp(YD_pot) = exp(CD_pot) + (1+g_fcn_pot) * exp(ID_pot);
...
exp(UCD_pot) = ( exp(CD_pot) - h*exp(CD_pot(-1))/exp(g_pot(-1)) )^(-1) - beta * h * ( exp(CD_pot(+1)) * exp(g_pot) - h * exp(CD_pot) )^(-1);
